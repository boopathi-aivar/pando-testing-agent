...
from graph.state import InvoiceState, PageResult
from utils.quality_gate import is_meaningful_markdown
...
def _try_markitdown_page(pdf_bytes: bytes, page_num: int) -> Optional[str]:
    """Run MarkItDown on a single-page PDF slice (preserves digital text layer)."""
    try:
...
def _try_rapidocr(img_bytes: bytes) -> tuple[str, float]:
    """OCR a page image. Returns (text, avg_confidence)."""
    try:
        ocr = _rapid_ocr()
