...
from utils.prompt_quality import check_prompt_coverage, prompt_diff
...
3. Optional generic_note — a global instruction that applies to the whole extraction
   (e.g. "transform all nulls to empty strings", "unwrap value/explanation/confidence wrappers")
4. The JSON schema

Update the prompt so future extractions follow that feedback.
Keep the same overall structure (role, IMPORTANT INSTRUCTIONS, EXTRACT THE FOLLOWING FIELDS).
...
- corrections: specific fields marked wrong with notes
- generic_note: a global instruction applying to the entire JSON
  (examples: replace all null with "", unwrap {value,explanation,confidence} to bare values,
   normalize mode casing, etc.)
...
- Do not invent values not supported by the text or notes (except structural transforms
  explicitly requested in generic_note, such as null→"").
...
def remap_extracted_json(
    ...
    response = llm.invoke(
        [
            ("system", REMAP_SYSTEM),
            ("human", json.dumps(payload, indent=2)[:180000]),
        ]
    )
...
def apply_field_corrections(
    ...
    *,
    ...
    """
    Apply user feedback: optional per-field corrections and/or a generic_note.
    Optionally refine session_prompt, remap JSON, re-validate.
    ...
    if refine_prompt and session_prompt:
        logger.info(
            "Refining session_prompt (corrections=%d, generic_note=%s)",
            len(corrections),
            bool(note),
        )
        before_prompt = session_prompt
        new_prompt = refine_session_prompt(
            session_prompt, schema, corrections, generic_note=note
        )
    ...
    logger.info(
        "Remapping extracted JSON (corrections=%d, generic_note=%s)",
        len(corrections),
        bool(note),
    )
    try:
