...
from utils.prompt_quality import check_prompt_coverage, match_schema_mapping
...
- Do NOT copy another carrier's identity blindly if the schema/sample imply a different context;
  keep useful style and field-writing patterns from the sample.
...
def prompt_gen_node(state: InvoiceState) -> dict:
    ...
    # Keep existing prompt on Docling / correction re-entry unless empty
    ...
    if not sample:
        return {
            ...
            "fatal_error": (
                ...
                "extraction prompt as a style reference — not a full library of prompts."
            ),
            "force_prompt_regen": False,
        }
    ...
    logger.info(
        "Generated session_prompt (%d chars) sample=%d mapping=%s coverage_ok=%s",
        len(text),
        len(sample),
        bool(mapping_text),
        reports["prompt_coverage"].get("ok"),
    )
    return {
        "session_prompt": text,
